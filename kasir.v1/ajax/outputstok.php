...
if($cek){
	...
	if (count($keyword) == 2) {
		$resultTipe = query("SELECT tb_stok_barang.id_tipe, merek, tipe, warna, memori, harga_beli, harga_jual, count(*) as stok FROM `tb_stok_barang` INNER JOIN tb_tipe using(id_tipe) WHERE merek = '$keyword[0]' AND tipe LIKE '$keyword[1]%' GROUP BY id_tipe, warna, memori ORDER by merek, tipe, warna, memori ASC"); 
	}else if(count($keyword) == 3){
		$resultTipe = query("SELECT tb_stok_barang.id_tipe, merek, tipe, warna, memori, harga_beli, harga_jual, count(*) as stok FROM `tb_stok_barang` INNER JOIN tb_tipe using(id_tipe) WHERE merek = '$keyword[0]' AND tipe = '$keyword[1]' AND warna LIKE '$keyword[2]%' GROUP BY id_tipe, warna, memori ORDER by merek, tipe, warna, memori ASC"); 
	}else if(count($keyword) == 4){
		$resultTipe = query("SELECT tb_stok_barang.id_tipe, merek, tipe, warna, memori, harga_beli, harga_jual, count(*) as stok FROM `tb_stok_barang` INNER JOIN tb_tipe using(id_tipe) WHERE merek = '$keyword[0]' AND tipe = '$keyword[1]' AND warna = '$keyword[2]' AND memori LIKE '$keyword[3]%' GROUP BY id_tipe, warna, memori ORDER by merek, tipe, warna, memori ASC"); 
	}

}else{
	$resultTipe = query("SELECT tb_stok_barang.id_tipe, merek, tipe, warna, memori, harga_beli, harga_jual, count(*) as stok FROM `tb_stok_barang` INNER JOIN tb_tipe using(id_tipe) WHERE merek LIKE '$keyword%' OR tipe LIKE '$keyword%' OR warna LIKE '$keyword%' OR memori LIKE '$keyword%' OR harga_beli LIKE '$keyword%' OR harga_jual LIKE '$keyword%' GROUP BY id_tipe, warna, memori ORDER by merek, tipe, warna, memori ASC"); 
}
